...
const PSE_NAME = 'PSE Email client';
const PSE_VERSION = '2.17.15';
...
function pseAccountSettingKeys(): array
{
  return [
    ...
    'suggest_unknown_read_contacts', 'confirm_delete_messages', 'compose_save_drafts', 'mobile_single_pane', 'app_title'
  ];
}
...
function pseSaveUploadedAppIcon(array $file): array
{
  ...
  if (function_exists('imagecreatefromstring') && function_exists('imagepng')) {
    ...
    $png = (string)ob_get_clean();
    imagedestroy($image);
  } elseif ($mime === 'image/png' && substr($raw, 0, 8) === "\x89PNG\r\n\x1a\n") {
    $png = $raw;
  } else {
    throw new RuntimeException('PHP GD is required to convert this image to icon.png. Upload a PNG instead.');
  }
...
function pseServePwaManifest(): void
{
  ...
  $name = trim((string)($settings['app_title'] ?? 'PSE Mail'));
  if ($name === '') {
    $name = 'PSE Mail';
  }
  ...
  $manifest = [
    ...
    'name' => $name,
    'short_name' => 'PSE',
    'description' => 'PSE Email client — private IMAP, SMTP and Gmail email client.',
...
<head>
  ...
  <meta name="apple-mobile-web-app-title" content="<?= htmlspecialchars((string)($pseSettings['app_title'] ?? 'PSE Mail'), ENT_QUOTES, 'UTF-8') ?>">
  ...
  <title><?= htmlspecialchars((string)($pseSettings['app_title'] ?? 'PSE Mail'), ENT_QUOTES, 'UTF-8') ?></title>
                ...
                <div class="col-12">
                  ...
                  <div class="pse-app-icon-upload">
                    ...
                    <div class="pse-app-icon-upload-copy">
                      ...
                      <div class="form-text mt-0">Upload PNG, JPEG, WebP, or GIF up to 5 MB. It is converted to PNG and replaces icon.png.</div>
                    </div>
                  ...
                  <div class="pse-pwa-settings-card p-3" id="pwaSettingsCard">
                    <div class="d-flex align-items-center gap-3">
                      ...
                      <div class="flex-grow-1 min-w-0">
                        <div class="fw-semibold">Install <?= htmlspecialchars((string)($pseSettings['app_title'] ?? 'PSE Mail'), ENT_QUOTES, 'UTF-8') ?></div>
  ...
  <script>
    (() => {
      ...
      const appearanceSettingKeys = [
        ...
        'confirm_delete_messages', 'compose_save_drafts', 'mobile_single_pane'
      ];
      ...
      function showMobileSwipeBackHint() {
        ...
        mobileSwipeHintTimer = setTimeout(() => hint.classList.remove('pse-show'), 2000);
      }
      ...
      function applySettingsToForm(settings) {
        ...
        const appTitle = String(effectiveSettings.app_title || 'PSE Mail');
      ...
      async function uploadApplicationIcon(file) {
        ...
        const form = new FormData();
        form.append('icon', file, file.name || 'icon');
        try {
          ...
          $('#appIconFile').value = '';
          toast('Application icon updated. icon.png has been replaced.');
        } catch (error) {
          $('#appIconFile').value = '';
          handleError(error);
        }
        ...
        return `
          <div class="pse-pwa-dialog">
            <div class="pse-pwa-dialog-hero">
              ...
              <div>
                <div class="pse-pwa-dialog-title">Install ${escapePwaHtml(String(initialSettings.app_title || 'PSE Mail'))}</div>
